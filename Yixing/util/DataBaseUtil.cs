...
namespace Yixing.util
{
   public class DataBaseUtil
    ...
    //数据库连接字符串
    public static string connectionString = "Database='airfoil';Data Source='localhost';User Id='root';Password='root';charset='utf8';pooling=true";
    ...
    /// <param name="commandParameters">执行命令所用参数的集合</param>
    /// <returns>执行命令所影响的行数</returns>
    public static int insertAndGetId(CommandType cmdType, string cmdText, params MySqlParameter[] commandParameters)
    {

        MySqlCommand cmd = new MySqlCommand();
       // cmdText += " select @@IDENTITY as lastInsertId";
        using (MySqlConnection conn = new MySqlConnection(connectionString))
        {
            PrepareCommand(cmd, conn, null, cmdType, cmdText, commandParameters);
            //MySqlParameter mp = cmd.Parameters.Add("@lastInsertId", MySqlDbType.Int32);
